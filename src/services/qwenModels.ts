import { decrementInFlight, getAllAccountEmails, getAccountCount, getTokenWithAccount } from './auth.ts';
...
export async function fetchQwenModels(): Promise<any[]> {
  if (getAccountCount() === 0 && process.env.TEST_MOCK_PLAYWRIGHT !== 'true') {
  ...
  return cachedModels || DEFAULT_QWEN_MODELS;
}
